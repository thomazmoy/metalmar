  ...
  <section class="w3l-about-breadcrumb">
    <div class="breadcrumb-bg breadcrumb-bg-contact py-5">
      <div class="container text-center py-lg-5 py-md-3">
        <h2>{{ GoogleTranslate::trans('Pesquisa',\App::getLocale()) }}</h2>
      </div>
    </div>
  </section>
        ...
        <div class="row justify-content-center mb-5">
          <div class="col-lg-6 w3l-contact text-center">
            <form method="GET" action="{{ route('pesquisar') }}" class="contact-form d-flex">
              <input type="text" class="form-control" name="pesquisar" id="pesquisar" placeholder="{{ GoogleTranslate::trans('Pesquisar',\App::getLocale()) }}" value="{{ Request('pesquisar')}}" /><br />
        ...
        <div class="row">
          ...
          @empty
            <div class="col-md-6 column pb-4 m-auto text-center">
              <h3 class="fw-bold text-verde pb-2">{{ GoogleTranslate::trans('Conteúdo Não Encontrado',\App::getLocale()) }}</h3>
              <p>{{ GoogleTranslate::trans('Tente mudar os parâmetros de busca.',\App::getLocale()) }}</p>
            </div>
          @endforelse
        </div>
