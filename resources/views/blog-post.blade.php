  ...
  <section class="w3l-about-breadcrumb">
    <div class="breadcrumb-bg breadcrumb-bg-contact py-5">
      <div class="container text-center py-lg-5 py-md-3">
        <h1 class="text-white fw-bold">{{ GoogleTranslate::trans(($post->titulo),\App::getLocale()) }}</h1>
      </div>
    </div>
  </section>
          ...
          <div class="col-lg-8 bg-branco rounded box-shadow-1 p-4">
            ...
            <div class="row my-3 justify-content-between">
              ...
              <div class="col-md-6 text-md-right">
                <p class="fw-bold text-azul-2"><span class="fa fa-tags mr-2"></span>{{ GoogleTranslate::trans(($post->categoria->nome),\App::getLocale()) }}</p>
              </div>
            </div>
            ...
            <div class="d-grid justify-content-center my-4">
              <h4 class="fw-bold py-2">{{ GoogleTranslate::trans('Compartilhe',\App::getLocale()) }}</h4>
          ...
          <div class="col-lg-4 p-4">
            <ul class="unlisted w3l-contact pb-4">
              <li class="border-bottom pb-3">
                <h3 class="fw-bold pb-2">{{ GoogleTranslate::trans('Pesquisar',\App::getLocale()) }}</h3>
                <form method="GET" action="{{ route('pesquisar') }}" class="contact-form d-flex">
                  <input type="text" class="form-control" name="pesquisar" id="pesquisar" placeholder="{{ GoogleTranslate::trans('Digite aqui',\App::getLocale()) }}" value="{{ Request('pesquisar')}}"><br>
                  <button class="btn btn-primary" type="submit"><i class="fa fa-search fs-4" aria-hidden="true"></i></button>
                </form>
              </li>
            </ul>
            <h3 class="fw-bold pb-2">{{ GoogleTranslate::trans('Posts Recentes',\App::getLocale()) }}</h3>
            <ul class="unlisted">
                ...
                @if($blogs->id != $post->id)
                  <li class="media border-bottom py-3">
                    <a href='{{ url("blog/{$blogs->urltitulo}") }}' class="d-flex align-items-center">
                      ...
                      @endif
                      <div class="d-block">
                        <h6 class="text-primary fw-bold pb-2">{{ GoogleTranslate::trans(($blogs->titulo),\App::getLocale()) }}</h6>
