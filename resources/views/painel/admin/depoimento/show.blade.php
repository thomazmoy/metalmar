        ...
        <div class="col-lg-8 mb-lg-0">
          <div class="card">
            <div class="card-header d-inline-flex pb-0 px-3">
              <h6 class="mb-0">Depoimento</h6>
              <a class="btn bg-gradient-ms ml-auto" href="{{ URL::previous() }}"><i class="fas fa-angle-left"></i>&nbsp;&nbsp;Voltar</a>
            </div>
            <div class="card-body p-4">
              <div class="d-flex flex-column h-100">
                <h5 class="font-weight-bolder">{{$depoimentos->nome}}</h5>
                <h6>{{$depoimentos->cargo}}</h6>
                <p>{{$depoimentos->texto}}</p>
                @if($depoimentos->img)
                  <img src='{{ url("storage/{$depoimentos->img}") }}' class="border-radius-lg" width="250px">
                @endif
              </div>
            </div>
          </div>
