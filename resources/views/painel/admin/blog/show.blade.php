          ...
          <div class="card">
            ...
            <div class="card-body p-4">
              <div class="d-flex flex-column h-100">
                <p class="mb-1 text-bold">{{strftime('%d %b %Y', strtotime($blogs->created_at))}}</p>
                <h5 class="font-weight-bolder">{{$blogs->titulo}}</h5>
                <h6>Url Amigável: {{ url("/blog/{$blogs->urltitulo}") }}</h6>
                <h6>Categoria: {{$blogs->categoria->nome}}</h6>
                <small class="form-text text-muted">Imagem</small>
                @if($blogs->img)
                  <img src='{{ url("storage/{$blogs->img}") }}' class="border-radius-lg d-lg-block mb-4" width="200px">
                @endif
                @if($blogs->img2)
                  <img src='{{ url("storage/{$blogs->img2}") }}' class="border-radius-lg d-lg-block mb-4" width="200px">
                @endif
                @if($blogs->img3)
                  <img src='{{ url("storage/{$blogs->img3}") }}' class="border-radius-lg d-lg-block mb-4" width="200px">
                @endif
                @if($blogs->imgwhats)
                  <img src='{{ url("storage/{$blogs->imgwhats}") }}' class="border-radius-lg d-lg-block" width="200px">
                @endif
                <h6 class="my-4">{!!$blogs->texto!!}</h6>
                <div class="row">
                  <div class="col-lg-4">
                    {!!$blogs->iframe!!}
                  </div>
