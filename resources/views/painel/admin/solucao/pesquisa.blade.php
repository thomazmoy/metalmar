...
<title>MS System | Soluções</title>
@section('content')
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Soluções</li>
          </ol>
        ...
        <div class="col-lg-12">
          <div class="card mb-4">
            <div class="card-header">
              <h6 class="float-left">Soluções</h6>
              <a class="btn bg-gradient-ms mb-0 float-right" href="{{route('solucao.create')}}"><i class="fas fa-plus"></i>&nbsp;&nbsp;Adicionar Novo</a>
            ...
            <div class="card-body px-0 pt-0 pb-2">
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    @foreach($pesquisa as $solucao)
                      <tr>
                        <td>
                          <div class="d-flex px-2 py-1">
                            <div class="d-flex flex-column justify-content-center">
                              @if($solucao->img)
                                <img src='{{ url("storage/{$solucao->img}") }}' class="avatar avatar-sm me-3">
                              @endif
                            </div>
                          </div>
                        </td>
                        <td>
                          <div class="d-flex px-2 py-1">
                            <div class="d-flex flex-column justify-content-center">
                              <h6 class="mb-0 text-sm">{{$solucao->titulo}}</h6>
                            </div>
                          </div>
                        </td>
                        ...
                        <td>
                          <a class="icon icon-shape bg-gradient-primary shadow text-center border-radius-md" href="{{route('solucao.show', $solucao->id)}}">
                            <i class="fas fa-eye text-lg opacity-10" aria-hidden="true"></i>
                          </a>
                          <a class="icon icon-shape bg-gradient-warning shadow text-center border-radius-md" href="{{route('solucao.edit', $solucao->id)}}">
                            <i class="fas fa-pencil-alt text-lg opacity-10" aria-hidden="true" ></i>
                          </a>
                          @if($solucao->id > '3')
                            <a type="submit" class=" icon icon-shape bg-gradient-danger shadow text-center border-radius-md" data-bs-toggle="modal" data-bs-target="#delete" data-orcid='{{$solucao->id}}'>
  ...
  <div class="modal fade" id="delete" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <form action="{{route('solucao.destroy','solucao')}}" method="post">
          {{method_field('delete')}}
          {{csrf_field('')}}
          <div class="modal-body">
            <p class="text-center">Você deseja realmente REMOVER?</p>
            <input type="hidden" name="solucao_id" id="orc_id" value="">
          </div>
