    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          ...
          <h6 class="font-weight-bolder mb-0">Pesquisa</h6>
        </nav>
        ...
        <div class="col-lg-12">
          <div class="card mb-4">
            <div class="card-header">
              <h6 class="float-left">Contato</h6>
              <a class="btn bg-gradient-ms mb-0 float-right" href="{{route('contato.create')}}"><i class="fas fa-plus"></i>&nbsp;&nbsp;Adicionar Novo</a>
                ...
                <table class="table align-items-center mb-0">
                  <thead>
                    <tr>
                      ...
                      <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Situação</th>
                  ...
                  <tbody>
                    @foreach($search as $contato)
                    <tr>
                      <td>
                        <div class="d-flex px-2 py-1">
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">{{$contato->nome}}</h6>
                          </div>
                        </div>
                      </td>
                      <td>
                        <div class="d-flex px-2 py-1">
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">{{$contato->email}}</h6>
                          </div>
                        </div>
                      </td>
                      <td>
                        <div class="d-flex px-2 py-1">
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">{{$contato->telefone}}</h6>
                          </div>
                        </div>
                      </td>
                      <td class="align-middle text-sm">
                        <span class="badge badge-sm bg-gradient-info">{{$contato->situacao}}</span>
                      </td>
                      <td>
                        <div class="d-flex px-2 py-1">
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">{{strftime('%d %b %Y', strtotime($contato->created_at))}}</h6>
                          </div>
                        </div>
                      </td>
                      <td>
                        <a class="icon icon-shape bg-gradient-primary shadow text-center border-radius-md" href="{{route('contato.show', $contato->id)}}">
                          <i class="fas fa-eye text-lg opacity-10" aria-hidden="true"></i>
                        </a>
                        <a class="icon icon-shape bg-gradient-warning shadow text-center border-radius-md" href="{{route('contato.edit', $contato->id)}}">
                          <i class="fas fa-pencil-alt text-lg opacity-10" aria-hidden="true" ></i>
                        </a>
                        <a type="submit" class=" icon icon-shape bg-gradient-danger shadow text-center border-radius-md" data-bs-toggle="modal" data-bs-target="#delete" data-orcid='{{$contato->id}}'>
          ...
          {{ $search->appends(Request::get('search'))->links() }}
        </div>
      </div>
      ...
      <div class="modal-content">
        ...
        <form action="{{route('contato.destroy','contato')}}" method="post">
          ...
          <div class="modal-body">
            ...
            <input type="hidden" name="contato_id" id="orc_id" value="">
          </div>
